...
static const char *labels[] = { "A","X","Y","Z","PC","SP","SR","CY","N","V","B","I","Z","C", NULL };
...
void DBGXRender(int *address,int showDisplay) {
	...
	DN(s->a,2);DN(s->x,2);DN(s->y,2);DN(s->z,2);DN(s->pc,4);DN(s->sp+0x100,4);DN(s->status,2);DN(s->cycles,4);
	DN(s->sign,1);DN(s->overflow,1);DN(s->decimal,1);DN(s->interruptDisable,1);DN(s->zero,1);DN(s->carry,1);
